...
- (void)setTitle:(NSString *)title {
    ...
    _titleLabel.frame = CGRectMake(UIScreen.mainScreen.bounds.size.width/2 - 100.0, CGRectGetMinY(self.imageView.frame) - 40.0 - height, 200.0, height);
}
...
- (void)setContentText:(NSString *)contentText {
    ...
    _contentLabel.frame = CGRectMake(UIScreen.mainScreen.bounds.size.width/2 - 100.0, CGRectGetMaxY(self.imageView.frame) + 20.0, 200.0, height);
}

- (void)setImage:(UIImage *)image {
    _image = image;
    self.imageView.image = image;
    self.imageView.tintColor = UIColorFromRGB(0x5199FF);
